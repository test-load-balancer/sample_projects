...
function red_bold {
    tput setaf 7; tput bold; tput setab 1; echo -n $1; tput sgr0
}
...
java -jar vendor/tlb_rb/tlb-all-gv0.2-9-g1f1a4aa.jar 2>/dev/null 1>/dev/null &
sleep 1
tlb_server_pid=$!
...
echo_note "Terminating the TLB server now, in a real situation TLB server would not be started and terminated every time partitions of a test-suite or
 or a single partition ends, insteed it'll continue running and can cater more than one project's test-suite. Its a shared server, and is not supposed
